...
export default function MailerRichEditorPage() {
  return (
    <Suspense fallback={null}>
      <MailerRichEditorPageInner />
    </Suspense>
  );
}

function MailerRichEditorPageInner() {
      ...
      try {
        ...
        const body = (await res.json().catch(() => null)) as {
          ...
          data?: {
            name: string;
            subject: string | null;
            editorProjectJson: { kind?: string; text?: string } | null;
          };
        ...
        if (typeof body.data.editorProjectJson?.text === "string") {
          setText(body.data.editorProjectJson.text);
        }
      } catch (err) {
        if (!cancelled) {
